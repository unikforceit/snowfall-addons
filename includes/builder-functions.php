...
class Woofall_Woo_Custom_Template_Layout
{
    ...
    public function init()
    {
        ...
        add_filter('wc_get_template_part', array($this, 'wf_get_product_page_template'), 99, 3);
        add_filter('template_include', array($this, 'wf_get_product_elementor_template'), 100);
        add_action('woofall_woocommerce_product_content', array($this, 'wf_get_product_content_elementor'), 5);
        add_action('woofall_woocommerce_product_content', array($this, 'wf_get_default_product_data'), 10);

        // Product Archive Page
        add_action('template_redirect', array($this, 'woofall_product_archive_template'), 999);
        add_filter('template_include', array($this, 'woofall_redirect_product_archive_template'), 999);
        add_action('woofall_woocommerce_archive_product_content', array($this, 'woofall_archive_product_page_content'));
    }
    ...
    {
        $template_id = woofall_option($field_key);
        if ('0' !== $template_id) {
            return $template_id;
        } else {
            return false;
    ...
    public function wf_get_product_elementor_template($template)
    {
        if (is_embed()) {
            return $template;
        }
        if (is_singular('product')) {
            if (Woofall_Woo_Custom_Template_Layout::wf_custom_product_template()) {
                ...
                if ('elementor_header_footer' === $templateid) {
                    ...
                    $template = WOOFALL_ADDONS_PL_PATH . 'woo-templates/single-product-canvas.php';
                }
            }
        }
        return $template;
    ...
    // product data
    public function wf_get_default_product_data()
    {
    ...
    * Archive Page
    */
    public function woofall_product_archive_template()
    {
        $archive_template_id = 0;
        if (defined('WOOCOMMERCE_VERSION')) {
            $termobj = get_queried_object();
            $get_all_taxonomies = woofall_get_taxonomies();
    ...
    public function woofall_redirect_product_archive_template($template)
    {
        $archive_template_id = $this->woofall_product_archive_template();
        $templatefile = array();
        ...
        if ($archive_template_id != '0') {
            ...
            if (!$template || (!empty($status_options['template_debug_mode']) && current_user_can('manage_options'))) {
                $template = WOOFALL_ADDONS_PL_PATH . 'woo-templates/archive-product.php';
            }
            $page_template_slug = get_page_template_slug($archive_template_id);
            if ('elementor_header_footer' === $page_template_slug) {
                $template = WOOFALL_ADDONS_PL_PATH . 'woo-templates/archive-product-fullwidth.php';
            } elseif ('elementor_canvas' === $page_template_slug) {
                $template = WOOFALL_ADDONS_PL_PATH . 'woo-templates/archive-product-canvas.php';
            }
        }
        return $template;
    }
    ...
    // Element Content
    public function woofall_archive_product_page_content($post)
    {
        $archive_template_id = $this->woofall_product_archive_template();
        if ($archive_template_id != '0') {
...
Woofall_Woo_Custom_Template_Layout::instance();